...
class _FavoritesScreenState extends State<FavoritesScreen> {
  ...
  Widget build(BuildContext context) {
    // Determine current filter name and icon for display in the AppBar
    String filterName = "All Flags";
    IconData filterIcon = Icons.filter_list; // Default icon (not filtered)
    // Check against 0 sentinel value for "All Flags"
    if (_selectedFilterFlagId != 0) {
      // Find the flag name from the current available list (_allAvailableFlags should be up-to-date)
      final flag = _allAvailableFlags.firstWhere(
          (f) => f.id == _selectedFilterFlagId,
          // Fallback if the selected flag ID somehow isn't in the list anymore
          // (e.g., it was deleted/hidden just before a rebuild)
          orElse: () {
              // If the selected flag is not found, reset the filter state
              // Use WidgetsBinding to schedule state update after build if needed
              WidgetsBinding.instance.addPostFrameCallback((_) {
                 if (mounted && _selectedFilterFlagId != 0) { // Check again to avoid loop
              ...
              return Flag(id:0, name:"All Flags"); // Return a temporary flag
          }
      );
       filterName = flag.name;
       // Check if flag was actually found (ID is not 0) after orElse check
       if (flag.id != 0) {
            filterIcon = Icons.filter_list_alt; // Use filled icon when filtered
       } else {
           // Reset name if orElse was triggered and ID became 0
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [ // Actions buttons in the AppBar
          // --- Filter Button ---
          Tooltip(
            message: "Filter by: $filterName",
            child: PopupMenuButton<int>( // Use non-nullable int
              icon: Icon(filterIcon), // Dynamic icon based on filter state
              initialValue: _selectedFilterFlagId, // Highlights current selection
              onSelected: (int selectedId) { // Receives non-nullable int value from PopupMenuItem
                print("[Filter Popup] Selected ID: $selectedId (Current State: $_selectedFilterFlagId)");
                if (_selectedFilterFlagId != selectedId) {
                  setState(() { // Update the filter state
                    _selectedFilterFlagId = selectedId;
                  });
                  // Reload the favorites list based on the new filter
                  _loadFavoritesAndApplyState();
                }
              },
              itemBuilder: (BuildContext context) {
                // Build the list of filter options dynamically
                List<PopupMenuEntry<int>> items = [];

                // Add "All Flags" option
                items.add(
                  PopupMenuItem<int>(
                    value: 0, // <<< Value passed to onSelected for "All Flags"
                    child: Text(
                      "All Flags",
                      style: TextStyle(
                ...
                items.add(const PopupMenuDivider()); // Separator

                // Add available flags from the state list
                if (_allAvailableFlags.isEmpty) {
                  ...
                  for (final flag in _allAvailableFlags) {
                    items.add(
                      PopupMenuItem<int>(
                        value: flag.id, // <<< Value passed to onSelected for this flag
                        child: Text(
                          flag.name,
                          style: TextStyle(
                            // Bold if currently selected
                ...
                return items; // Return the fully built list
              }, // End itemBuilder
            ), // End PopupMenuButton
          ), // End Tooltip
          // --- End Filter Button ---
            ...
            onSelected: (SortOption selectedOption) {
              if (_currentSortOption != selectedOption) {
                setState(() {
                  ...
                  _applySort(); // Re-sort the currently displayed list
                });
              }
            },
            itemBuilder: (BuildContext context) => <PopupMenuEntry<SortOption>>[
              // Menu items for sorting options
      ...
      body: _isLoading
          ...
          // Check original fetched list (_allFetchedFavorites) for the initial empty message when NO filter is applied
          : _allFetchedFavorites.isEmpty && _selectedFilterFlagId == 0
              ...
              : _displayFavorites.isEmpty
                  ...
                  // If we have favorites to display
                  : RefreshIndicator(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          // Use the extracted FavoriteListItemCard widget
                          return FavoriteListItemCard(
                            favoriteVerse: favorite,
                            // Pass the function to get the full book name
                            getFullBookName: getFullBookName,
                            // Pass callback for removing this specific favorite
                            onRemove: () => _removeFavorite(favorite.verseID),
                            // Pass callback for managing flags for this specific favorite
