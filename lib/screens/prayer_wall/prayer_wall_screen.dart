...
import '../../widgets/prayer_wall/starfield_background.dart'; // NEW IMPORT
import '../../widgets/prayer_wall/celestial_light_rays.dart'; // NEW IMPORT
import '../../widgets/prayer_wall/flowing_light_background.dart'; // NEW IMPORT
...
class _PrayerWallScreenState extends State<PrayerWallScreen>
  ...
  void initState() {
    ...
    _prayerStreamSubscription =
        prayerService.getApprovedPrayers(limit: 20).listen(
      (prayers) {
        if (!mounted) return;
    ...
    _userAuthSubscription =
      ...
      if (mounted) {
        ...
        if (userChanged || _currentUserPrayerProfile == null) {
          // Fetch if user changed or profile not yet loaded
  ...
  Future<void> _fetchUserPrayerStreakData() async {
    ...
    if (_currentUser != null) {
      // Works for anonymous or logged-in users
      final prayerService = Provider.of<PrayerService>(context, listen: false);
      _currentUserPrayerProfile =
          await prayerService.getUserPrayerProfile(_currentUser!.uid);
    } else {
  ...
  @override
  void dispose() {
    // Unchanged (except added _userAuthSubscription.cancel())
  ...
  void _updateRiverPrayersList(List<PrayerRequest> newPrayersFromStream) {
    ...
    if (_animatingOutPrayerId != null) {
      animatingPrayerIds.add(_animatingOutPrayerId!);
    }
    ...
    if (listActuallyChanged ||
        (_riverPrayers.isEmpty && newVisuals.isNotEmpty)) {
      setState(() {
        _riverPrayers = newVisuals;
        if (_currentPage >= _riverPrayers.length) {
          _currentPage =
              _riverPrayers.isNotEmpty ? max(0, _riverPrayers.length - 1) : 0;
        }
  ...
  void _handlePrayForRequest(RiverPrayerVisual riverPrayerVisual) {
    ...
    _moteOverlayEntry = OverlayEntry(builder: (context) {
      ...
      final startRenderBox = riverPrayerVisual.itemKey.currentContext
          ?.findRenderObject() as RenderBox?;
      final endRenderBox =
          _wellOfHopeKey.currentContext?.findRenderObject() as RenderBox?;
      if (startRenderBox == null ||
          !startRenderBox.hasSize ||
          endRenderBox == null ||
          !endRenderBox.hasSize) {
        _activeMoteAnimation?.controller.dispose();
        _activeMoteAnimation = null;
        WidgetsBinding.instance.addPostFrameCallback((_) {
          _moteOverlayEntry?.remove();
          _moteOverlayEntry = null;
          if (mounted && _animatingOutPrayerId == prayerRequest.prayerId) {
            setState(() {
              _animatingOutPrayerId = null;
            });
          }
        });
      ...
      return AnimatedPrayerMote(
        ...
        startPosition: startOffset +
            Offset(
                startRenderBox.size.width / 2, startRenderBox.size.height / 2),
    ...
    _activeMoteAnimation!.controller.forward().then((_) async {
      ...
      if (wellState != null) {
        final RenderBox? wellRenderBox =
            _wellOfHopeKey.currentContext?.findRenderObject() as RenderBox?;
        if (wellRenderBox != null && wellRenderBox.hasSize) {
          final wellCenterGlobal = wellRenderBox.localToGlobal(Offset(
              wellRenderBox.size.width / 2, wellRenderBox.size.height / 2));
      ...
      if (mounted) {
        int removedIndex = _riverPrayers
            .indexWhere((p) => p.prayer.prayerId == prayerRequest.prayerId);
        if (removedIndex != -1) {
          ...
          if (_riverPrayers.isEmpty)
            _currentPage = 0;
          else if (_currentPage >= _riverPrayers.length)
            _currentPage = max(0, _riverPrayers.length - 1);
          setState(() {
            _animatingOutPrayerId = null;
          });
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (mounted && _pageController.hasClients) {
              ...
              if (_riverPrayers.isEmpty)
                ...
                targetPage = max(0, _riverPrayers.length - 1);
              if (_riverPrayers.isNotEmpty &&
                  _pageController.page?.round() != targetPage) {
                _pageController.jumpToPage(targetPage);
              } else if (_riverPrayers.isEmpty &&
  ...
  void _handleReportPrayer(PrayerRequest prayerRequest) {
    ...
    if (_currentUser == null) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Please log in to report prayers.')));
      return;
    }
    _autoCycleTimer?.cancel();
    _isUserInteractingWithPage = true;
    showPrayerReportDialog(
        ...
        onSubmitReport: (reason) async {
          ...
          bool success =
              await prayerService.reportPrayer(prayerRequest.prayerId, reason);
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                content:
                    Text(success ? 'Prayer reported.' : 'Failed to report.')));
            if (success) {
              int removedIndex = _riverPrayers.indexWhere(
                  (p) => p.prayer.prayerId == prayerRequest.prayerId);
              if (removedIndex != -1) {
                _riverPrayers.removeAt(removedIndex);
                if (_currentPage >= removedIndex && _currentPage > 0) {
                  _currentPage = max(0, _currentPage - 1);
      ...
      if (mounted && _isUserInteractingWithPage) {
        _isUserInteractingWithPage = false;
        _resetAutoCycleTimer();
      } else if (mounted) {
        _resetAutoCycleTimer();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Prayer Wall of Hope',
      ...
      body: Stack(
        children: [
          ...
          // Layer 2: Starfield (upper portion — twinkling stars)
          const Positioned.fill(
            child: StarfieldBackground(starCount: 50),
          ),

          // Layer 3: Celestial Light Rays (behind the Well)
          const Positioned.fill(
            child: CelestialLightRays(),
          ),

          // Layer 4: The Mystical Embers (existing, enhanced)
          const Positioned.fill(
            child: MysticalEmbersBackground(),
          ),

          // Layer 5: Flowing Light Background (behind prayer cards)
          const Positioned.fill(
            child: FlowingLightBackground(),
          ),

          // Layer 6: The Content (Existing SafeArea and Children)
          SafeArea(
            bottom: false,
            child: Column(
              children: [
                ...
                Expanded(
                  flex: 2,
                  child: WellOfHopeWidget(
                      key: _wellOfHopeWidgetStateKey, wellKey: _wellOfHopeKey),
                ),
                Expanded(
                  flex: 2,
                  child: Container(
                    ...
                    child: _riverPrayers.isEmpty &&
                            _animatingOutPrayerId == null
                        ? Center(
                            child: (_prayerStreamSubscription == null &&
                                    _riverPrayers.isEmpty &&
                                    !Provider.of<PrayerService>(context,
                                            listen: false)
                                        .toString()
                                        .contains("Instance"))
                                  ...
                                  child: ConstrainedBox(
                                    ...
                                    child: Padding(
                                      ...
                                      child: RiverPrayerItem(
                                        ...
                                        onTap: () =>
                                            _handlePrayForRequest(riverPrayer),
                                        onLongPress: () => _handleReportPrayer(
                                            riverPrayer.prayer),
                                        animation: riverPrayer.animation,
                                        playExitAnimation:
                                            _animatingOutPrayerId ==
                                                riverPrayer.prayer.prayerId,
                                      ),
                                    ),
                                  ),
                                );
                ...
                Padding(
                  ...
                  child: Text(
                    _riverPrayers.isNotEmpty
                        ? "Tap a prayer to send your support. Swipe to see more."
                        : "Tap a prayer to send support.",
                    textAlign: TextAlign.center,
